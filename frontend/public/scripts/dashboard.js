...
If context is relevant, use it.
  ...
  function renderCaloriesPage() {
    ...
    if (!list) return;  // at minimum, need the list element

    // Only update charts if their canvas elements are present
    const chartsAvailable = summary && calorieCanvas && macroCanvas;
...
Do not include any explanation, ONLY the JSON object. Use realistic average values for this food item.`
  ...
  if (manualFoodForm) {
    manualFoodForm.addEventListener('submit', (e) => {
      ...
      if (!foodName || isNaN(calories) || calories <= 0 || isNaN(protein) || isNaN(carbs) || isNaN(fat)) {
        alert('Please fill in food name and calories (must be > 0). Protein, carbs and fat are required too.');
        return;
      }
      ...
      // Scroll food list into view
      const foodListCard = document.querySelector('.food-list-card');
      if (foodListCard) {
        setTimeout(() => foodListCard.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      }
